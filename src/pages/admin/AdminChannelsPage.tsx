          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  선택한 채널의 자막 컴포넌트 위치만 투명 오버레이로 확인합니다. 전체 화면 검토는 URL에 `view=full`을 붙여 확인합니다.
                </p>
              </div>
              <Link
                to={selectedPreviewPath}
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-3">
              {livePreviewLanguages.map((language) => {
                const livePath = selectedDisplay
                  ? `/live/${selectedChannel.slug}/${language.code}?layoutId=${selectedDisplay.layoutId}`
                  : `/live/${selectedChannel.slug}/${language.code}`;
                ...
                return (
                  <article key={language.code} className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                    ...
                    <div className="aspect-video bg-slate-950">
                      <iframe
                        src={livePath}
                        title={`${language.label} caption preview`}
