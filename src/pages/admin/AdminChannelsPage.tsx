...
export default function AdminChannelsPage() {
  ...
  const selectedFullPreviewPath = selectedDisplay
    ? `/live/${mockPlatformData.channel.slug}/${selectedDisplay.defaultLanguageCode}?layoutId=${selectedDisplay.layoutId}&view=full`
    : `/live/${mockPlatformData.channel.slug}/en?view=full`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-indigo-600">Overlay Verification</p>
            <h3 className="text-lg font-bold text-slate-900">언어별 자막 화면 확인</h3>
            <p className="mt-1 text-sm text-slate-500">
              선택한 채널의 레이아웃에서 자막 컴포넌트 위치만 투명 오버레이로 확인합니다. 전체 화면 검토는 URL에 `view=full`을 붙여 확인합니다.
            </p>
          </div>
          <Link
            to={selectedPreviewPath}
